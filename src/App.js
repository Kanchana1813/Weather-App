...
function App() {
  ...
  const API_KEY = 'a682117da9aab41b6f623bc67672383c';
    ...
    if (event.key === 'Enter') {
      if (location) {
        axios.get(url).then((response) => {
          ...
          if (response.data.name && !searchHistory.includes(response.data.name)) {
            const newSearchHistory = [response.data.name, ...searchHistory.slice(0, 4)];
          ...
          {showSearchHistory && searchHistory.length > 0 && (
            <ul className="search-history-list">
                {searchHistory.map((location) => (
                  <li key={location} onClick={() => handleSearchHistoryClick(location)}>
                    {location}
                  </li>
                ))}
            </ul>
        ...
        <h2>Favorites</h2>
          <ul>
            {favoriteLocations.map((location) => (
              <li key={location} onClick={() => handleFavLocationClick(location)} >
                {location}{' '}
                <button id='remove' onClick={() => handleRemoveFromFavorites(location)}>
                <FaTrash />
                </button>
              </li>
            ))}
